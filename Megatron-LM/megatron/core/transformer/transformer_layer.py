...
class MeKi(torch.nn.Module):
    """
    Implement MeKi module with transformer_engine
    Currently only TP=1 is supported
    """
    def __init__( self, config, submodules=None ):

        super().__init__()

        assert config.tensor_model_parallel_size == 1, "Only support TP=1 now."
        from megatron.core.transformer.mlp import MLP
        from megatron.core.extensions.transformer_engine import TEColumnParallelLinear, TELayerNormColumnParallelLinear, TERowParallelLinear
        from transformer_engine.pytorch import RMSNorm as te_RMSNorm
        
        self.mix_norm = te_RMSNorm(
            hidden_size=config.meki_dim,
            eps=config.layernorm_epsilon,
            sequence_parallel=config.sequence_parallel,
            zero_centered_gamma=config.layernorm_zero_centered_gamma,
        )
        self.alpha_scale = torch.nn.Parameter(torch.tensor(1.))
        self.beta_scale = torch.nn.Parameter(torch.tensor(1.))

        mlp_module = deepcopy(submodules.mlp.submodules)
        meki_mlp_cfg = deepcopy(config)
        meki_mlp_cfg.ffn_hidden_size = config.hidden_size // 2
        meki_mlp_cfg.hidden_size = config.meki_dim
        self.word_emb_projection = MLP(meki_mlp_cfg, mlp_module, input_size=config.hidden_size)

        self.embeddings = tensor_parallel.VocabParallelEmbedding(
            num_embeddings=config.padded_vocab_size,
            embedding_dim=config.meki_dim,
    ...
    def forward(self, hidden_states, input_ids, word_emb):
        
        static_embedding = self.embeddings(input_ids).transpose(0, 1)
        dynamic_embedding, _ = self.word_emb_projection(word_emb)
        
        meki_embedding = self.mix_norm(
            static_embedding + dynamic_embedding * self.beta_scale
        )  * self.alpha_scale

        hidden_states, _ = self.gate_proj(hidden_states)

        fused = torch.nn.functional.sigmoid(hidden_states).add(meki_embedding)

        fused, _ = self.out_proj(fused)
        
        output_mkei = self.post_norm(fused)
...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def __init__(
        ...
        from transformer_engine.pytorch import RMSNorm as te_RMSNorm
        self.pre_mlp_layernorm = te_RMSNorm(
            hidden_size=config.hidden_size,
            eps=config.layernorm_epsilon,
            sequence_parallel=config.sequence_parallel,
            zero_centered_gamma=config.layernorm_zero_centered_gamma,
        ...
        # [Module 8: MLP block]
        _submodules_mlp = deepcopy(submodules.mlp)
        _submodules_mlp.submodules.linear_fc1 = TEColumnParallelLinear
        self.mlp = build_module(_submodules_mlp, config=self.config)
        ...
        # MeKi module init ===========================================
        self.meki_module = None
        if self.config.meki_dim > 0:
            self.meki_module = MeKi(config, submodules)
        #=================================================================
    ...
    def forward(
        ...
        **kwargs
        ...
        output = self._forward_mlp(input_ids, word_emb, pre_mlp_layernorm_output, residual)
    ...
    def _forward_mlp(self, input_ids, word_emb, pre_mlp_layernorm_output, residual):
        """
        ...
        # ==================================
        # make MeKi module parallel to MLP
        if self.meki_module:
            assert input_ids is not None and word_emb is not None
            assert mlp_output_with_bias[1] is None, "mlp.bias=True not supported"

            output_mkei = self.meki_module(pre_mlp_layernorm_output, input_ids, word_emb)

            mlp_output_with_bias = (output_mkei + mlp_output_with_bias[0], None)
        #==================================
