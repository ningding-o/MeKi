...
def pretrain(
    ...
    # print meki model #param
    print_rank_0(model[0])
    total_num_param = sum([p.numel() for p in model[0].parameters()])
    print_rank_0(f"Total #param (active + embedding): {total_num_param / 10**9:.2f} B")
    is_emb_param = lambda x: any([i>50000 for i in x.shape])
    emb_num_param = sum([p.numel() for p in model[0].parameters() if is_emb_param(p)])
    print_rank_0(f"Embedding #param : {emb_num_param / 10**9:.2f} B")
    print_rank_0(f"Active #param : {(total_num_param - emb_num_param) / 10**9:.2f} B")
    # done
